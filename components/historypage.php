...
<ul class="nav nav-pills mb-5 bg-light">
  ...
  <li class="nav-item">
    <a class="nav-link active" href = "http://localhost/Grievence-System/components/historypage.php?prn=<?php echo $prn ?>">History</a>
  </li>
</ul>
...
<div class = "container">
    ...
    <?php
        ...
        $sql = "SELECT * FROM complaint WHERE status = 'Completed' AND prn_number = '{$prn}'";
        ...
        if($result->num_rows > 0){
            ...
            while($row = $result->fetch_assoc()){
                echo "<tr>";
                    ...
                    echo "<td><a href = 'http://localhost/Grievence-System/components/history.php?ticket=$row[ticket_number]&prn=$row[prn_number]&fname=$row[fname]&mname=$row[mname]&lname=$row[lname]&branch=$row[branch]&degree=$row[degree]&complaints=$row[complaints]&status=$row[status]&catagory=$row[catagory_complaint]&solution=$row[solution]' class = 'btn btn-primary'>View</a></td>";
                echo "</tr>";
            }
        }
        else{
